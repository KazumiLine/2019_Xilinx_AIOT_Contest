...
def processLiveFeed(cur, nxt, pframe, cframe):
    ...
    w = framebbox[2]
    h = framebbox[3]
    kp1, des1 = orb.detectAndCompute(cur[y : y + h, x : x + w], None)
    ...
    s_w = sbbox[2]
    s_h = sbbox[3]
    kp2, des2 = orb.detectAndCompute(nxt[s_y : s_y + s_h, s_x : s_x + s_w], None)
    ...
    if not ((des1 is None) or (des2 is None)):
        ...
        M = videoDrawMatches(
            frame_i, a, kp1, frame_ipp, b, kp2, matches, 0, framebbox, cur, s_i
        )
